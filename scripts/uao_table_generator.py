...
struct Big5_UTF8_Table {
    ...
    char utf8[UTF8_SIZE];
        ...
        b2u.sort(key=lambda x: x[0])

        print('struct Big5_UTF8_Table BIG5_TO_UTF8_TABLE[] = {', file=f)
        for x in b2u:
            print('    {{ "{}", "{}", }},'.format(get_byte_string(x[0]), get_byte_string(x[1])), file=f)
        print('};', file=f)

        b2u.sort(key=lambda x: x[1])

        print('struct Big5_UTF8_Table UTF8_TO_BIG5_TABLE[] = {', file=f)
        for x in b2u:
            print('    {{ "{}", "{}", }},'.format(get_byte_string(x[0]), get_byte_string(x[1])), file=f)
        print('};', file=f)
